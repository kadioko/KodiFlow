-- Allow renewal to change billing frequency while keeping the old frequency as the default.

DROP FUNCTION IF EXISTS public.renew_lease(UUID, DATE, NUMERIC);
DROP FUNCTION IF EXISTS public.renew_lease(UUID, DATE, NUMERIC, NUMERIC);
DROP FUNCTION IF EXISTS public.renew_lease(UUID, DATE, NUMERIC, NUMERIC, TEXT);
...
CREATE OR REPLACE FUNCTION public.renew_lease(
  ...
  p_new_billing_frequency TEXT DEFAULT NULL
)
...
BEGIN
  ...
  _new_billing_frequency := COALESCE(p_new_billing_frequency, '');

  IF _new_billing_frequency = '' THEN
    _new_billing_frequency := NULL;
  END IF;

  IF _new_billing_frequency IS NOT NULL
    AND _new_billing_frequency NOT IN ('monthly', 'quarterly', 'semi_annually', 'annually')
  THEN
    RAISE EXCEPTION 'Billing frequency must be monthly, quarterly, semi_annually, or annually';
  END IF;
  ...
  _new_billing_frequency := COALESCE(_new_billing_frequency, _old_lease.billing_frequency);
