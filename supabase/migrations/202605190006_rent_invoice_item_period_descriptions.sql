-- Include the full billing period in generated rent invoice item descriptions.
...
BEGIN
  ...
  IF _start_date > _lease_end THEN
    RAISE EXCEPTION 'Billing period starts after the lease term';
  END IF;

  _end_date := LEAST((_start_date + (_months || ' months')::INTERVAL - INTERVAL '1 day')::DATE, _lease_end);
