...
DECLARE
  ...
  _months INTEGER;
  _billing_index INTEGER;
  _lease_start_index INTEGER;
...
BEGIN
  ...
  _lease_start_index := EXTRACT(YEAR FROM _lease_start)::INTEGER * 12 + EXTRACT(MONTH FROM _lease_start)::INTEGER - 1;
  _billing_index := p_billing_year * 12 + p_billing_month - 1;
  _months_since_start := _billing_index - _lease_start_index;

  IF _months_since_start < 0 THEN
    RAISE EXCEPTION 'Selected billing period starts before this lease begins';
  END IF;
  ...
  IF _start_date > _lease_end THEN
    RAISE EXCEPTION 'Selected billing period starts after this lease ends';
  END IF;

  IF _end_date > _lease_end THEN
    _end_date := _lease_end;
  END IF;

  SELECT ri.id INTO _new_invoice_id
  FROM public.rent_invoices ri
  WHERE ri.user_id = _uid
    AND ri.tenant_id = _tenant_id
    AND ri.unit_id = _unit_id
    AND ri.status <> 'cancelled'
    AND daterange(ri.billing_period_start, ri.billing_period_end, '[]')
  ...
  LOOP
    _charge_amount := CASE
      ...
      WHEN _charge.frequency = 'quarterly' THEN CASE WHEN _months >= 3 THEN COALESCE(_charge.amount, 0) * FLOOR(_months / 3) ELSE 0 END
      WHEN _charge.frequency = 'annually' THEN CASE WHEN _months >= 12 THEN COALESCE(_charge.amount, 0) * FLOOR(_months / 12) ELSE 0 END
      WHEN _charge.frequency = 'custom' THEN COALESCE(_charge.amount, 0)
  ...
  VALUES (
    ...
    _start_date,
    _end_date,
    EXTRACT(MONTH FROM _start_date)::INTEGER,
    EXTRACT(YEAR FROM _start_date)::INTEGER,
    _subtotal,
    _due_date,
    'unpaid'
  )
  ...
  VALUES (
    ...
    _new_invoice_id,
    CASE
      WHEN _months = 1 THEN 'Rent from ' || to_char(_start_date, 'DD Mon YYYY') || ' to ' || to_char(_end_date, 'DD Mon YYYY')
      ELSE _months || '-Month Rent from ' || to_char(_start_date, 'DD Mon YYYY') || ' to ' || to_char(_end_date, 'DD Mon YYYY')
    END,
    'rent',
